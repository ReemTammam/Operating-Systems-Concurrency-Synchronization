...
This document maps each requirement from Task 1 to the design and

algorithmic decisions used in the implementation. The goal is to ensure

that every part of the implementation directly satisfies a requirement
...
All synchronization must be implemented using semaphores. Other

mechanisms such as synchronized blocks, locks, atomics, or built-in

barrier constructs are not allowed.
...
\- Each philosopher is represented by a thread instance.
...
Chopsticks must be shared resources that philosophers acquire before
...
The system must avoid deadlock situations where philosophers hold one

chopstick and wait indefinitely for another.



\### Implementation Mapping

\- Chopstick acquisition order is modified for at least one philosopher
...
\- The last arriving thread releases permits allowing all threads to
...
\- Meal counters and shared state variables are protected using a mutex

&nbsp; semaphore to ensure atomic updates.
...
Busy waiting loops are not allowed except for simulating eating or

thinking for random durations.
...
\- Output statements are placed directly at the corresponding algorithm

&nbsp; steps within the philosopher thread execution.
...
\- A shared meal counter is checked and updated inside a semaphore-

&nbsp; protected critical section before eating begins.
...
The implementation follows the Dining Philosophers algorithm while

ensuring correct synchronization using semaphores only. Each design

decision directly corresponds to a requirement in Task 1.
